...
- Each card shows:
  ...
  - **Delta + trend** — below the value, color-coded: green for `up` (positive), red for `down` (negative), gray for `flat`.
...
- `trend` semantics: `up` means "good direction" (green), `down` means "bad direction" (red). For metrics where down is good (e.g., churn), the LLM should set `trend: up` when churn decreases. Alternatively, we could add a `positive: up|down` field to flip the color mapping, but this adds complexity — start simple.
- Custom CSS variables: `--glyph-kpi-trend-up` (green), `--glyph-kpi-trend-down` (red), `--glyph-kpi-trend-flat` (gray).
