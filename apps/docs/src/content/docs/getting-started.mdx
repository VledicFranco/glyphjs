...
<Tabs>
  ...
  <TabItem label="pnpm">
    ```bash
    pnpm add @glyphjs/compiler @glyphjs/runtime @glyphjs/components
    ```
  </TabItem>
  <TabItem label="yarn">
    ```bash
    yarn add @glyphjs/compiler @glyphjs/runtime @glyphjs/components
    ```
  </TabItem>
</Tabs>
...
const runtime = createGlyphRuntime({
  ...
  onInteraction: debounceInteractions((event) => {
    console.log(event.kind, event.blockId, event.payload);
  }, 300),
});
```

See the [Authoring Guide](/glyphjs/authoring-guide/#interaction-events) for the full list of supported events and payload structures.
...
- [**Components**](/glyphjs/components) — Browse all 21 built-in components for content, data, diagrams, and interactivity.
- [**Interaction Events**](/glyphjs/authoring-guide/#interaction-events) — Enable event emission from interactive blocks for analytics or LLM integration.
