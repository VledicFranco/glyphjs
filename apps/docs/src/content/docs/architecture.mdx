...
The parser extends [remark](https://github.com/remarkjs/remark) with a custom plugin that recognizes `ui:*` fenced code blocks. Standard Markdown is parsed normally; fenced blocks with the `ui:` prefix are extracted as `GlyphUIBlock` AST nodes with their YAML payload preserved.
...
The compiler uses a **collect-all-errors** strategy: IR is always produced, even when errors exist.
