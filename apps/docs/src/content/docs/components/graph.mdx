...
export const dagExample = `\`\`\`ui:graph
interactive: true
type: dag
nodes:
  - id: api
...
<GlyphPreview client:only="react" source={dagExample} interactive />
...
export const mindmapExample = `\`\`\`ui:graph
type: mindmap
nodes:
  - id: root
    label: Glyph JS
...
<GlyphPreview client:only="react" source={mindmapExample} />
...
:::note[Breaking Change in v0.4]
Prior to v0.4, the default behavior was `always`. The new default is `modifier-key` for better UX in scrollable content. To restore the old behavior, explicitly set `interactionMode: always`. See the [Migration Guide](/migration-v0.4) for details.
:::

## Shared abstraction with Relation

The Graph and [Relation](/glyphjs/components/relation/) components share a unified relational abstraction. Both use nodes and edges, but `ui:relation` constrains the model by requiring `cardinality` on edges and treating nodes as entities with typed attributes.
...
:::note
When a graph has both `interactive: true` and cross-block `refs`, clicking a navigable node fires **both** the `onNavigate` callback (for reference resolution) and the `onInteraction` callback (for event reporting). They do not conflict.
:::
