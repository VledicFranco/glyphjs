...
<GlyphPreview client:only="react" source={tableExample} interactive />
...
export const narrowTableExample = `\`\`\`ui:table
columns:
  - key: name
    label: Employee
    sortable: true
...
<div style="max-width: 420px; border: 1px solid var(--sl-color-gray-5); border-radius: 8px; padding: 0.5rem;">
  <GlyphPreview client:only="react" source={narrowTableExample} />
</div>
...
:::tip
Table filter events can fire rapidly as the user types. Use `debounceInteractions` from `@glyphjs/runtime` to batch them.
:::
