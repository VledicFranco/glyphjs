...
For `ui:*` blocks, `data` is `Record<string, unknown>` -- the parsed YAML payload from the Markdown source after `glyph-id` and `refs` keys have been stripped. The shape is validated at runtime by the component Zod schema. See the [Plugin API](/glyphjs/reference/plugin-api/) for how schemas are defined.
